              ...
              toast.success("The order has been created", {
                description: (
                  <div className="flex flex-col gap-0.5">
                    <p>An account has been created with the information you provided.</p>
                    <div className="bg-muted relative p-4">
                      <pre>{userInfo}</pre>
                      <Button
                        variant="outline"
                        size="icon"
  ...
  return (
    <div className="flex grow gap-2 py-2 max-sm:flex-col">
      {shoppingCartItems.length > 0 ? (
        <>
          <aside className="flex w-1/2 flex-col gap-4 px-2 max-sm:w-full">
            {status === AuthStatus.UNAUTHENTICATED ? (
              ...
              <div className="flex grow flex-col">
                <h4 className="text-3xl font-bold">Your personal data</h4>
                <ul>
                  {(Object.entries(FIELDS_CONFIG) as Entries<typeof FIELDS_CONFIG>).map(
                    ([name, { label }]) => (
                      <li key={name}>
        ...
        <p className="m-auto px-8 text-center text-4xl">
          There are currently no items in your cart. Please add at least one item.
        </p>
      )}
    </div>
  );
